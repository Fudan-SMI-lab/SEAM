...
# ── repair_role_descriptions placeholder tests ─────────────────────────

def test_error_recovery_prompt_renders_repair_role_descriptions() -> None:
    """phase_error_recovery.md renders {repair_role_descriptions} placeholder with provided context."""
    loader = PromptLoader(prompts_dir=str(PROMPTS_DIR))
    context = {
        **COMMON_CONTEXT,
        "phase_name": "error_analyzer",
        ...
        "repair_role_descriptions": (
            "## Repair Roles\n"
            "- `dependency_fixer`: Fix packages.\n"
            "- `code_adapter`: Fix Python code.\n"
            "- `operator_fixer`: Fix native operators.\n"
            "- `final_gate_report_fixer`: Fix report schema.\n"
        ),
    }
    prompt = loader.load_prompt("phase_error_recovery", context)
    assert "final_gate_report_fixer" in prompt
    assert "Fix report schema" in prompt
    assert "<dependency_fixer | code_adapter | operator_fixer>" not in prompt
    assert "selected repair role from available roles below" in prompt


def test_error_recovery_prompt_uses_generic_repair_role_description() -> None:
    """phase_error_recovery.md uses generic 'selected repair role from available roles below' wording."""
    content = (PROMPTS_DIR / "phase_error_recovery.md").read_text(encoding="utf-8")
    assert "selected repair role from available roles below" in content
    assert "<dependency_fixer | code_adapter | operator_fixer>" not in content


def test_error_recovery_container_prompt_uses_generic_repair_role_description() -> None:
    """phase_error_recovery_container.md uses generic wording and {repair_role_descriptions} placeholder."""
    content = (PROMPTS_DIR / "phase_error_recovery_container.md").read_text(encoding="utf-8")
    assert "selected repair role from available roles below" in content
    assert "<dependency_fixer | code_adapter | operator_fixer>" not in content
    assert "{repair_role_descriptions}" in content
    # The old hardcoded block should NOT appear
    assert "Fix CUDA-to-NPU code migration at the Python level" not in content


def test_error_recovery_musa_prompt_uses_generic_repair_role_and_placeholder() -> None:
    """phase_error_recovery_container_musa.md uses generic wording and {repair_role_descriptions}."""
    content = (PROMPTS_DIR / "phase_error_recovery_container_musa.md").read_text(encoding="utf-8")
    assert "selected repair role from available roles below" in content
    assert "{repair_role_descriptions}" in content


def test_error_recovery_ppu_prompt_uses_generic_repair_role_and_placeholder() -> None:
    """phase_error_recovery_container_ppu.md uses generic wording and {repair_role_descriptions}."""
    content = (PROMPTS_DIR / "phase_error_recovery_container_ppu.md").read_text(encoding="utf-8")
    assert "selected repair role from available roles below" in content
    assert "{repair_role_descriptions}" in content
    # The old hardcoded PPU-specific block should NOT appear
    assert "Fix Python-level API replacements, device placement" not in content


def test_repair_role_descriptions_text_includes_report_fixer_when_provided() -> None:
    """_repair_role_descriptions_text includes final_gate_report_fixer when in available_roles."""
    text = repair_loop._repair_role_descriptions_text({"dependency_fixer", "code_adapter", "operator_fixer", "final_gate_report_fixer"})
    assert "final_gate_report_fixer" in text
    assert "Report schema" in text or "report aggregation" in text.lower()

    text_no_report = repair_loop._repair_role_descriptions_text({"dependency_fixer", "code_adapter", "operator_fixer"})
    assert "final_gate_report_fixer" not in text_no_report
