...
def test_task7_context_management_uses_loaded_typed_config_and_honors_disabled(
    tmp_path: Path,
) -> None:
    executor, _, _, _ = _build_task7_loop_executor(tmp_path)
    typed = load_context_management_config(
        _task7_feature_config({"enabled": False, "keep_recent_turns": 0})[
            "context_management"
        ]
    )
    executor.framework_config = {"context_management": typed}

    assert executor._context_config() is typed
    assert executor._context_budget_active() is False


def test_task7_zero_recent_turns_keeps_no_loop_prompt_history(
    tmp_path: Path,
) -> None:
    executor, _, _, _ = _build_task7_loop_executor(tmp_path)
    executor.framework_config = _task7_feature_config({"keep_recent_turns": 0})

    assert executor._bounded_loop_history([{"iteration": 1}]) == []
