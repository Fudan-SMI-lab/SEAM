...
try:
    import _sqlite3  # noqa: F401  # pylint: disable=unused-import; silent
except NameError:
    _sqlite3 = None  # type: ignore[misc, assignment]  # pylint: disable=invalid-name; silent
...
class FakeSessionManager(MigrationSessionManager):
    ...
    def _http(  # pylint: disable=too-many-arguments,too-many-positional-arguments; silent
        self,
        method: str,
        path: str,
...
def _manager_with_message(
    message: Response, history: RouteValue | None = None, status_type: str = "idle"
) -> FakeSessionManager:
    return FakeSessionManager(
        {
            ("POST", "/session/ses-1/message"): {"ok": True, "data": message},
            ("GET", "/session/status"): {"ok": True, "data": {"ses-1": {"type": status_type}}},
            ("GET", "/session/ses-1/message"): history
            or {"ok": True, "data": [{"todos": [{"status": "completed"}]}]},
        }
    )


def _sqlite_backed_manager(
    db_path: Path, status_data: dict[str, Any] | None = None
) -> FakeSessionManager:
    manager = FakeSessionManager(
        {
            ...
            ("GET", "/session/status"): {
                "ok": True,
                "data": status_data if status_data is not None else {},
            },
            ("GET", "/session/ses-1/message"): {
                "ok": True,
                "data": [{"parts": [{"type": "text", "text": "No structured todo list."}]}],
            },
        }
    )
...
def test_detect_agent_prefers_exact_atlas_then_contains_atlas() -> None:
    ...
    exact._detect_agent()  # pylint: disable=protected-access; silent
    ...
    containing._detect_agent()  # pylint: disable=protected-access; silent
...
def test_send_command_rejects_compaction_response_as_incomplete() -> None:
    ...
    assert not any(
        call["method"] == "GET" and call["path"] == "/session/status" for call in manager.calls
    )
...
def test_send_command_recovers_empty_post_response_from_latest_history() -> None:
    manager = _manager_with_message(
        ...
        history=[
            ...
            {
                "ok": True,
                "data": [{"parts": [{"type": "text", "text": "recovered phase complete"}]}],
            },
        ],
    )
...
def test_send_command_waits_for_incomplete_todos_until_complete(
    ...
    manager = _manager_with_message(
        ...
        history=[
            ...
            {
                "ok": True,
                "data": [{"todos": [{"status": "in_progress", "content": "rerun validator"}]}],
            },
            {
                "ok": True,
                "data": [{"todos": [{"status": "completed", "content": "rerun validator"}]}],
            },
        ],
    )
    ...
    status_calls = [
        ...
        if call["method"] == "GET" and call["path"] == "/session/status"
    ]
    assert result == "phase complete"
...
def test_send_command_times_out_for_incomplete_todos_without_reposting(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manager = _manager_with_message(
...
def test_sqlite_fallback_ignores_unrelated_incomplete_todos(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        conn.execute(
            'INSERT INTO todos ("sessionID", status, content) VALUES (?, ?, ?)',
            ("other-session", "pending", "other work"),
        )
        conn.execute(
            'INSERT INTO todos ("sessionID", status, content) VALUES (?, ?, ?)',
            ("ses-1", "completed", "own work"),
        )
...
@_SKIP_SQLITE
def test_sqlite_fallback_blocks_camelcase_session_pending_todo(
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        conn.execute(
            'INSERT INTO tasks ("sessionID", status, content) VALUES (?, ?, ?)',
            ("ses-1", "pending", "rerun validator"),
        )
        conn.execute(
            'INSERT INTO tasks ("sessionID", status, content) VALUES (?, ?, ?)',
            ("other-session", "completed", "other work"),
        )
...
@_SKIP_SQLITE
def test_sqlite_idle_session_with_pending_todo_is_incomplete(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    db_path = tmp_path / "opencode.db"
    with sqlite3.connect(db_path) as conn:
...
def test_sqlite_idle_session_with_completed_todos_is_complete(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        conn.execute(
            'INSERT INTO todos ("sessionID", status, content) VALUES (?, ?, ?)',
            ("ses-1", "completed", "rerun validator"),
        )
...
@_SKIP_SQLITE
def test_send_command_timeout_none_uses_sqlite_assistant_completion_without_todos(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "opencode.db"
    assistant_data = {
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        conn.execute(
            'CREATE TABLE message ("sessionID" TEXT, role TEXT, data TEXT, timeCreated INTEGER)'
        )
        conn.execute(
            "INSERT INTO session (id, title, time_compacting) VALUES (?, ?, ?)",
            ("ses-1", "migration-main_engineer", None),
        )
        conn.execute(
...
@_SKIP_SQLITE
def test_sqlite_assistant_completion_still_blocks_same_session_pending_todo(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    db_path = tmp_path / "opencode.db"
    assistant_data = {
...
@_SKIP_SQLITE
def test_sqlite_assistant_completion_still_blocks_active_compaction(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    db_path = tmp_path / "opencode.db"
    assistant_data = {
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        conn.execute(
            "INSERT INTO session (id, title, time_compacting) VALUES (?, ?, ?)",
            ("ses-1", "migration-main_engineer", 1),
        )
        conn.execute(
            'INSERT INTO message ("sessionID", role, data, timeCreated) VALUES (?, ?, ?, ?)',
            ("ses-1", "assistant", json.dumps(assistant_data), 2),
...
def test_sqlite_fallback_skips_todo_tables_without_session_column(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        conn.execute(
            "INSERT INTO todos (status, content) VALUES (?, ?)", ("pending", "unscoped work")
        )
...
def test_pending_word_in_normal_text_does_not_mark_todos_incomplete() -> None:
    manager = _manager_with_message(
        {
            "info": {"finish": "stop"},
            "parts": [{"type": "text", "text": "The pending import issue was resolved."}],
        },
        history={
            "ok": True,
            "data": [
                {
                    "parts": [
                        {"type": "text", "text": "No structured todos. Pending issue resolved."}
                    ]
                }
            ],
        },
    ...
    assert (
        ...
        == "The pending import issue was resolved."
    )
...
def test_send_command_surfaces_auth_error_after_observed_idle(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manager = FakeSessionManager(
        {
...
def test_wait_for_idle_returns_idle_when_status_empty_and_no_todos(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
class TestFetchAgentList:
    def test_returns_sorted_names_from_agent_endpoint(self) -> None:
        ...
        names = manager._fetch_agent_list()  # pylint: disable=protected-access; silent
    ...
    def test_returns_empty_list_on_non_list_data(self) -> None:
        manager = FakeSessionManager({("GET", "/agent"): {"ok": True, "data": "not_a_list"}})
        # pylint: disable-next=protected-access,use-implicit-booleaness-not-comparison; silent
        assert manager._fetch_agent_list() == []
    ...
    def test_skips_non_dict_entries(self) -> None:
        ...
        assert manager._fetch_agent_list() == ["Atlas"]  # pylint: disable=protected-access; silent
...
class TestOverrideAgent:
    ...
    def test_raises_for_invalid_name(self) -> None:
        manager = FakeSessionManager({("GET", "/agent"): {"ok": True, "data": [{"name": "build"}]}})
        with pytest.raises(ValueError, match="not found"):
            manager.override_agent("Atlas")


class TestAvailableAgentsProperty:  # pylint: disable=too-few-public-methods; silent
