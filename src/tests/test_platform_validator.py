...
class TestValidateCustomOpFinalGate:
    ...
    def test_ppu_full_pass_real_evidence(self):  # pylint: disable=too-many-locals; silent
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            assert result["passed"], (
                f"Expected PASS for PPU valid evidence, got errors: {result.get('errors')}"
            )
    ...
    def test_musa_accepts_generic_library_path_with_platform_build_proof(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            reports = root / "migration_reports"
            reports.mkdir()
            (reports / "migration_manifest.json").write_text(
                json.dumps({"required_units": ["op_generic_musa"]})
            )
            ...
            gate = {
                ...
                "performance_report": {
                    ...
                    "unit_count": 1,
                    "entries": {
                        "op_generic_musa": row["performance_evidence"]
                        | {"unit_identity": "op_generic_musa"}
                    },
            ...
            assert result["passed"], (
                f"Expected PASS for generic .so path with MUSA proof, got: {result.get('errors')}"
            )
...
class TestHasTargetDeviceCustomProof:
    ...
    def test_npu_policy_accepts_npu_custom_boolean(self):
        assert _has_target_device_custom_proof({"npu_custom": True}, BUILTIN_PRESETS["npu_ascend"])
...
class TestPathHasPlatformArtifactSignal:
    ...
    def test_platform_signal_none_fallback(self):
        ...
        assert _path_has_platform_artifact_signal("opp/custom_op/build/libcustom.so", None)

    def test_platform_signal_none_fallback_rejects_ppu(self):
        assert not _path_has_platform_artifact_signal("ppu_custom/build/libppu_op.so", None)
...
class TestRepairLoopEnginePlatformPolicy:
    ...
    def _mock_engine(policy=None):
        from unittest.mock import MagicMock  # pylint: disable=import-outside-toplevel; silent
    ...
    def test_validate_gate_passes_with_ppu_policy(self):  # pylint: disable=too-many-locals; silent
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (build_dir / "build.log").write_text(
                "nvcc -shared -o libppu.so kernel.cu -lcuda\nppu_compiler: ok\n"
            )
            so_path = build_dir / "libppu.so"
            so_path.write_bytes(
                b"\x7fELF\x02\x01\x01\x00" + b"\x00" * 128 + b"ppukernel\x00cuda\x00"
            )
            src_dir = root / "ppu_custom" / "src"
            src_dir.mkdir(parents=True)
            ...
            assert result["passed"] is True, (
                f"Expected PASS with PPU policy, got errors: {result.get('errors')}"
            )
...
class TestPerformanceValidationModes:
    ...
    def test_full_mode_rejects_missing_speedup(self):
        ...
        assert result["passed"] is False
        assert any("speedup" in e for e in result.get("errors", [])), (
            f"Expected speedup error, got: {result.get('errors')}"
        )
    ...
    def test_presence_only_accepts_without_speedup_when_timing_present(self):
        ...
        result = validate_custom_op_final_gate(gate, project_root=root, platform_policy=policy)
        assert result["passed"] is True, (
            f"Expected PASS in presence_only mode, got: {result.get('errors')}"
        )
    ...
    def test_presence_only_still_requires_positive_timing(self):
        ...
        result = validate_custom_op_final_gate(gate, project_root=root, platform_policy=policy)
        assert result["passed"] is False
        assert any("missing" in e or "positive" in e for e in result.get("errors", [])), (
            f"Expected positive timing error, got: {result.get('errors')}"
        )
    ...
    def test_disabled_skips_performance_but_rejects_missing_no_fallback(self):
        ...
        assert result["passed"] is False
        assert any(
            "must contain evidence" in e or "no_fallback_no_zero_call" in str(e)
            for e in result.get("errors", [])
        ), f"Expected no-fallback error, got: {result.get('errors')}"
    ...
    def test_disabled_accepts_missing_performance_evidence_field(self):
        ...
        result = validate_custom_op_final_gate(gate, project_root=root, platform_policy=policy)
        assert result["passed"] is True, (
            f"Expected PASS in disabled mode without perf evidence, got: {result.get('errors')}"
        )
...
class TestUnitIdentityMatching:
    ...
    def test_unit_identity_matching_passes_with_name_equals_unit_identity(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = validate_custom_op_final_gate(gate, project_root=root, platform_policy=ppu)
            assert result["passed"] is True, (
                f"Expected PASS with unit_identity-only rows, got: {result.get('errors')}"
            )
