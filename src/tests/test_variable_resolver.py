...
class TestBasicResolution:
    ...
    def test_int_parse(self, resolver):  # pylint: disable=redefined-outer-name; silent
        assert resolver.resolve("123") == 123

    def test_float_parse(self, resolver):  # pylint: disable=redefined-outer-name; silent
        assert resolver.resolve("3.14") == 3.14

    def test_bool_parse(self, resolver):  # pylint: disable=redefined-outer-name; silent
        assert resolver.resolve("true") is True
        assert resolver.resolve("false") is False

    def test_state_nested_path(self, resolver):  # pylint: disable=redefined-outer-name; silent
        result = resolver.resolve("${state.a.b.c}", state={"a": {"b": {"c": "deep"}}})
        assert result == "deep"

    def test_globals_key(self, resolver):  # pylint: disable=redefined-outer-name; silent
        result = resolver.resolve("${globals.max}", globals={"max": 5})
        assert result == 5

    def test_context_key(self, resolver):  # pylint: disable=redefined-outer-name; silent
        result = resolver.resolve("${context.PROJECT_DIR}", context={"PROJECT_DIR": "/tmp/test"})
        assert result == "/tmp/test"

    def test_loop_vars(self, resolver):  # pylint: disable=redefined-outer-name; silent
        result = resolver.resolve(
            "${loop_vars.entry_script}", loop_vars={"entry_script": "python main.py"}
        )
        assert result == "python main.py"

    def test_loop_state(self, resolver):  # pylint: disable=redefined-outer-name; silent
        result = resolver.resolve("${loop_state.exit_code}", loop_state={"exit_code": 1})
        assert result == 1

    def test_loop_history(self, resolver):  # pylint: disable=redefined-outer-name; silent
        history = [{"iter": 1}, {"iter": 2}]
        result = resolver.resolve("${loop_history}", loop_history=history)
        assert result == history

    def test_step_outputs(self, resolver):  # pylint: disable=redefined-outer-name; silent
        result = resolver.resolve(
            "${error_analysis.repair_role}",
...
class TestDefaultFilter:
    def test_default_when_missing(self, resolver):  # pylint: disable=redefined-outer-name; silent
        result = resolver.resolve("${missing_var | default 99}")
        assert result == 99

    def test_default_when_present(self, resolver):  # pylint: disable=redefined-outer-name; silent
        result = resolver.resolve("${context.X | default fallback}", context={"X": "present"})
        assert result == "present"

    def test_default_string_value(self, resolver):  # pylint: disable=redefined-outer-name; silent
        result = resolver.resolve("${missing | default 'hello'}")
        assert result == "hello"

    def test_default_bool_value(self, resolver):  # pylint: disable=redefined-outer-name; silent
...
class TestStateWildcard:
    ...
    def test_state_wildcard_empty(self, resolver):  # pylint: disable=redefined-outer-name; silent
...
class TestResolveDict:
    def test_resolve_dict_nested(self, resolver):  # pylint: disable=redefined-outer-name; silent
    ...
    def test_resolve_dict_list(self, resolver):  # pylint: disable=redefined-outer-name; silent
