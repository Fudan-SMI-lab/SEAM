        ...
        if isinstance(variants, list) and variants:
            ...
            for item in variant_items[:50]:
                if isinstance(item, dict):
                    ...
                    if unit_identity:
                        unit_ids.append(str(unit_identity))
                        continue
                    item = item_dict
...
def _reports_lack_unit_identities(inventory: object, manifest: object, gate: object) -> bool:
    ...
    for data in (inventory, manifest, gate):
        for entry in _candidate_entries(data):
            ...
            value = entry_dict.get("unit_identity")
...
def _strict_progress_lines(ledger: dict[str, object]) -> list[str]:
    ...
    if isinstance(strict_units, list) and strict_units:
        strict_unit_items = cast(list[object], strict_units)
        lines.append("Strict Pass Unit Identities: " + ", ".join(str(unit) for unit in strict_unit_items[:50]))
    remaining_units = ledger.get("remaining_units")
    if isinstance(remaining_units, list) and remaining_units:
        remaining_unit_items = cast(list[object], remaining_units)
        lines.append("Remaining Unit Identities: " + ", ".join(str(unit) for unit in remaining_unit_items[:50]))
    ...
    if isinstance(raw_units, list):
        ...
        for item in unit_items:
            if not isinstance(item, dict):
                continue
            item_dict = cast(dict[str, object], item)
            if item_dict.get("status") == "strict_pass":
                continue
            unit = str(item_dict.get("unit_identity") or "").strip()
            missing = item_dict.get("missing_evidence")
            missing_items = [str(value) for value in cast(list[object], missing)[:6]] if isinstance(missing, list) else []
