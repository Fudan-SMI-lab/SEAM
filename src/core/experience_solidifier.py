...
class SkillSolidifier(BaseSolidifier):
    ...
    def solidify(self, exp: dict, artifact_ctx: dict, run_id: str, store: Any) -> dict:
        skill_name = (
            exp.get("skill_name")
            or exp.get("name")
            or self._slug(exp.get("title", "skill"))
        )
    ...
    def _render_skill_markdown(self, data: dict) -> str:
        ...
        if data.get("code_changes"):
            lines.extend(
                ["", "## Code Examples", json.dumps(data.get("code_changes"), indent=2)]
            )
    ...
    def _render_verification(self, exp: dict, run_id: str) -> str:
        lines = [
            f"# Verification for {exp.get('title', 'Skill')}",
            "",
            f"- Source run: {run_id}",
        ]
        ...
        lines.append(
            "- Verify by applying the fix steps to a matching migration failure."
        )
        return "\n".join(lines) + "\n"
