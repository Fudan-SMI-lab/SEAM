...
def validate_custom_op_final_gate(  # pylint: disable=too-many-branches,too-many-locals; silent
    ...
    if (
        ...
        # pylint: disable-next=superfluous-parens; silent
...
def _validate_gate_row(
    ...
    if custom_call_count is None or custom_call_count <= 0:
        errors.append(
            # pylint: disable-next=consider-using-f-string; silent
            "rows[%d].same_run_runtime_coverage must include custom call count > 0" % index
        )
...
def _has_evidence(value: object) -> bool:  # pylint: disable=too-many-return-statements; silent
    if value is True:
        return True
...
def _validate_required_manifest_units(
    ...
    if row_names != required_set:
        ...
        errors.append(
            "rows must exactly match migration_manifest.required_units (" + "; ".join(details) + ")"
        )
...
def _load_required_manifest_units(project_root: Path | None, errors: list[str]) -> list[str]:
    ...
    try:
        if not manifest_path.is_file():
            errors.append(
                # pylint: disable-next=line-too-long; silent
                "migration_reports/migration_manifest.json must exist for custom-op final gate validation"
            )
            return []
        if manifest_path.stat().st_size > _MAX_MANIFEST_BYTES:
            errors.append(
                # pylint: disable-next=line-too-long; silent
                "migration_reports/migration_manifest.json is too large for custom-op final gate validation"
            )
            return []
    ...
    if len(units) != len(required_units):
        errors.append(
            # pylint: disable-next=line-too-long; silent
            "migration_reports/migration_manifest.json required_units must contain only non-empty strings"
        )
        return []
...
def _validate_fine_grained_inventory_entry(
    ...
    for field_name in ("native_operator_symbols", "kernel_functions", "kernel_launch_sites"):
        ...
        if isinstance(value, Mapping):
            errors.append(
    label +
     f".{field_name} must describe one fine-grained unit, not nested family/group mappings" )
    ...
    if unit_identity and row_name and unit_identity != row_name:
        errors.append(
            label + ".unit_identity must match the source_inventory/manifest row identity"
        )
...
def _validate_source_inventory_metadata(value: object, errors: list[str]) -> None:
    if not isinstance(value, Mapping):
        errors.append(
            # pylint: disable-next=line-too-long; silent
            "source_inventory must include discovery_complete and discovery_sources_checked metadata"
        )
        return
    inventory = cast(Mapping[object, object], value)
    ...
    if "out_of_scope_source_groups" not in inventory:
        errors.append(
            # pylint: disable-next=line-too-long; silent
            "source_inventory.out_of_scope_source_groups must list excluded source groups, even when empty"
        )
    ...
    if not isinstance(sources, list):
        ...
        return
    source_values = {
        str(source).strip().lower().replace("-", "_") for source in cast(list[object], sources)
    }
    if "requirements_doc" in source_values:
        errors.append(
            # pylint: disable-next=line-too-long; silent
            "source_inventory.discovery_sources_checked must be source-driven and must not include requirements_doc as a completion source"
        )
...
def _validate_project_local_artifact(
    ...
    if not isinstance(value, Mapping):
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"rows[{index}].opp_custom_op_artifact_evidence must be an object with project-local proof"
        )
        return
    evidence = cast(Mapping[object, object], value)
    if evidence.get("project_local") is not True and evidence.get("in_project") is not True:
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"rows[{index}].opp_custom_op_artifact_evidence must prove project-local artifact creation"
        )
    ...
    if _is_python_shim_artifact(evidence):
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"rows[{index}].opp_custom_op_artifact_evidence must not be a Python shim or Python binding surface"
        )
    ...
    if not native_paths:
        if platform_policy is not None and platform_policy.id != "npu_ascend":
            ...
            errors.append(
                # pylint: disable-next=line-too-long; silent
                f"rows[{index}].opp_custom_op_artifact_evidence must prove a native compiled {native_label} custom-op artifact"
            )
        else:
            errors.append(
                # pylint: disable-next=line-too-long; silent
                f"rows[{index}].opp_custom_op_artifact_evidence must prove a native compiled Ascend custom-op artifact"
            )
    elif not _runtime_loaded_native_artifact_matches(evidence, native_paths, project_root):
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"rows[{index}].opp_custom_op_artifact_evidence must prove the same-run runtime loaded the native compiled artifact, not a Python shim"
        )
    ...
    if project_root is not None and native_paths:
        ...
        if not existing_paths:
            errors.append(
                # pylint: disable-next=line-too-long; silent
                f"rows[{index}].opp_custom_op_artifact_evidence native artifact path must exist under the project root and be a non-empty compiled binary"
            )
...
def _validate_integration_route(value: object, index: int, errors: list[str]) -> None:
    ...
    if _mapping_is_disallowed_surrogate(evidence):
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"rows[{index}].integration_e2e_evidence must not be synthetic/monkeypatch/report/benchmark-only"
        )
    if not _has_positive_boolean(
        evidence, ("project_api_invoked", "public_api_invoked", "custom_op_route_executed")
    ):
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"rows[{index}].integration_e2e_evidence must prove public/project API custom-op execution"
        )
    if not _has_positive_boolean(
        evidence,
        (
        ...
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"rows[{index}].integration_e2e_evidence must prove native compiled custom-op route execution"
        )
...
def _validate_runtime_coverage(value: object, index: int, errors: list[str]) -> None:
    ...
    if _mapping_is_disallowed_surrogate(evidence):
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"rows[{index}].same_run_runtime_coverage must not be synthetic/monkeypatch/report/benchmark-only"
        )
    ...
    if not _has_positive_boolean(
        ...
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"rows[{index}].same_run_runtime_coverage must prove runtime coverage through the project API route"
        )
    if not _has_positive_boolean(
        evidence,
        (
        ...
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"rows[{index}].same_run_runtime_coverage must prove native compiled custom-op runtime coverage"
        )
...
def _validate_performance(
    ...
    if _mapping_is_disallowed_surrogate(evidence):
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"rows[{index}].performance_evidence must not be report-only or benchmark-only without project API proof"
        )
    ...
    if not _has_positive_boolean(
        ...
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"rows[{index}].performance_evidence must prove timing came from public/project API custom-op route"
        )
...
def _validate_performance_report_completeness(
    ...
    if not isinstance(report, Mapping):
        errors.append(
            # pylint: disable-next=line-too-long; silent
            "performance_report must be an object proving complete migration_reports/performance.json coverage"
        )
        return
    ...
    if _mapping_is_disallowed_surrogate(report_map):
        errors.append(
            # pylint: disable-next=line-too-long; silent
            "performance_report must not be report-only, benchmark-only, synthetic, mock, or manifest-only"
        )
    if not _has_positive_boolean(
        report_map,
        ("project_api_invoked", "public_api_invoked", "custom_op_route_executed", "verified"),
    ):
        errors.append(
            # pylint: disable-next=line-too-long; silent
            "performance_report must prove speedup timings came from public/project API custom-op routes"
        )
    _validate_baseline_and_custom_device_proof(
    ...
    if row_names:
        ...
        if report_names != row_names:
            ...
            errors.append(
                "performance_report must match manifest rows (" + "; ".join(details) + ")"
            )
...
def _validate_overall_performance_report(
    ...
    if perf_mode == "full":
        ...
        if missing:
            errors.append(
                "performance_report missing positive overall speedup fields: " + ", ".join(missing)
            )
        ...
        if missing:
            errors.append(
                "performance_report missing positive overall timing fields: " + ", ".join(missing)
            )
    ...
    if not route_proven:
        errors.append(
            # pylint: disable-next=line-too-long; silent
            "performance_report must prove overall timing ran through the project API after all custom-op units were replaced"
        )
    if not all_units_replaced_proven:
        errors.append(
            # pylint: disable-next=line-too-long; silent
            "performance_report must prove overall timing was measured after all source-discovered custom-op units were replaced"
        )
...
def _validate_performance_report_entry(
    ...
    if perf_mode == "full":
        if not _positive_number(entry.get("speedup_vs_baseline")):
            errors.append(
                # pylint: disable-next=line-too-long; silent
                f"performance_report.entries[{unit_name}].speedup_vs_baseline must be a positive number"
            )
    if not _has_positive_boolean(
        ...
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"performance_report.entries[{unit_name}] must prove public/project API custom-op timing route"
        )
    _validate_baseline_and_custom_device_proof(
...
def _validate_no_fallback_evidence(value: object, index: int, errors: list[str]) -> None:
    if not isinstance(value, Mapping):
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"rows[{index}].no_fallback_no_zero_call_no_builtin_contamination must be an object with explicit no-fallback proof"
        )
        return
    evidence = cast(Mapping[object, object], value)
    if _mapping_is_disallowed_surrogate(evidence):
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"rows[{index}].no_fallback_no_zero_call_no_builtin_contamination must not be synthetic/monkeypatch/report/benchmark-only"
        )
    ...
    if not all_negative_flags_false:
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"rows[{index}].no_fallback_no_zero_call_no_builtin_contamination must explicitly set all fallback/zero-call/builtin/baseline/stub flags to false"
        )
...
def _paths_match(runtime_path: str, native_path: str, project_root: Path | None) -> bool:
    if project_root is not None:
        ...
        if native_resolved is not None and _container_runtime_path_matches_project_relative(
...
def _validate_build_provenance(
    ...
    if not isinstance(provenance, Mapping):
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"rows[{index}].opp_custom_op_artifact_evidence must include build_provenance with command and project-local log_path"
        )
        return None
    ...
    if not isinstance(command, str) or not command.strip():
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"rows[{index}].opp_custom_op_artifact_evidence.build_provenance.command must be non-empty"
        )
    log_path = (
    ...
    if not isinstance(log_path, str) or not _is_safe_project_relative_path(log_path):
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"rows[{index}].opp_custom_op_artifact_evidence.build_provenance.log_path must be a safe project-relative path"
        )
        return None
    if project_root is not None:
        ...
        if resolved is None or not resolved.is_file() or resolved.stat().st_size <= 0:
            errors.append(
                # pylint: disable-next=line-too-long; silent
                f"rows[{index}].opp_custom_op_artifact_evidence.build_provenance.log_path must exist under the project root and be non-empty"
            )
            return None
        return resolved
    return None
...
def _validate_native_platform_evidence(
    ...
    ", ".join(
        sorted(evidence_only_paths)) )
    if not (binary_has_native_evidence or source_has_native_evidence):
        errors.append(f"rows[{index}].opp_custom_op_artifact_evidence {binary_source_error}")
...
def _validate_baseline_and_custom_device_proof(
    ...
    if not _has_baseline_proof(evidence, platform_policy):
        ...
        errors.append(
            # pylint: disable-next=line-too-long; silent
            f"{label} must prove timings include a baseline path ({', '.join(sorted(baseline_devices))})"
        )
...
def _has_negative_contamination_signal(value: object) -> bool:
    ...
    return (
        isinstance(perf_value, (int, float)) and not isinstance(perf_value, bool) and perf_value < 0
    )
