...
FINE_GRAINED_GRANULARITY_VALUES = {
    "FINE_GRAINED",
    "FINE_GRAINED_UNIT",
    "UNIT",
    "UNIT_LEVEL",
...
def validate_custom_op_final_gate(
    ...
    if (
        ...
        and closed_pass_entries is not None
    ):
        if not inventory_count == manifest_entries == closed_pass_entries:
            errors.append(
                "inventory_count, manifest_entries, and closed_pass_entries must match"
            )
        if inventory_count <= 0:
            errors.append(
                "inventory_count, manifest_entries, and closed_pass_entries must be > 0"
            )
    ...
    if not isinstance(rows, list) or not rows:
        ...
        for index, row_obj in enumerate(row_items):
            ...
            _validate_gate_row(
                row, index, errors, resolved_project_root, platform_policy
            )
        _validate_source_inventory_completeness(
            cast(Mapping[object, object], data), row_items, errors
        )
        perf_mode = get_performance_validation_mode(platform_policy)
        if perf_mode != "disabled":
            _validate_performance_report_completeness(
                cast(Mapping[object, object], data),
                row_items,
                manifest_entries,
                errors,
                platform_policy,
...
def _validate_gate_row(
    ...
    if perf_mode != "disabled":
        _validate_performance(
            row.get("performance_evidence"), index, errors, platform_policy
        )
    _validate_no_fallback_evidence(
        row.get("no_fallback_no_zero_call_no_builtin_contamination"), index, errors
    )

    custom_call_count = _extract_custom_call_count(row)
    if custom_call_count is None or custom_call_count <= 0:
        errors.append(
            "rows[%d].same_run_runtime_coverage must include custom call count > 0"
            % index
        )
    ...
    if _has_negative_contamination_signal(contamination):
        errors.append(
            f"rows[{index}].no_fallback_no_zero_call_no_builtin_contamination reports contamination"
        )
...
def _mapping_reports_failure(evidence: Mapping[object, object]) -> bool:
    ...
    return (
        normalized_status in BLOCKING_STATUSES
        or normalized_status not in EVIDENCE_PASS_STATES
    )
...
def _validate_source_inventory_completeness(
    ...
    if not inventory_names:
        errors.append(
            "source_inventory must contain source-discovered entries for every manifest row"
        )
        return
    ...
    for index, row_obj in enumerate(row_items):
        ...
        if not row_name:
            errors.append(
                f"rows[{index}] must identify the manifest custom op for source inventory matching"
            )
            continue
        ...
        if entry is None:
            continue
        _validate_native_inventory_entry(
            entry, f"source_inventory.entries[{row_name}]", errors
        )
    ...
    if row_names and inventory_names != row_names:
        ...
        if extra:
            details.append(
                "source inventory entries without manifest rows: " + ", ".join(extra)
            )
        errors.append(
            "source_inventory must match manifest rows (" + "; ".join(details) + ")"
        )
...
def _validate_required_manifest_units(
    ...
    if row_names != required_set:
        ...
        errors.append(
            "rows must exactly match migration_manifest.required_units ("
            + "; ".join(details)
            + ")"
        )
    ...
    if mismatched:
        errors.append(
            f"custom-op final gate counts must equal "
            f"migration_manifest.required_units length ({expected_count}): "
            f"{', '.join(mismatched)}"
        )


def _load_required_manifest_units(
    project_root: Path | None, errors: list[str]
) -> list[str]:
    ...
    try:
        if not manifest_path.is_file():
            errors.append(
                "migration_reports/migration_manifest.json must exist for "
                "custom-op final gate validation"
            )
            return []
        if manifest_path.stat().st_size > _MAX_MANIFEST_BYTES:
            errors.append(
                "migration_reports/migration_manifest.json is too large for "
                "custom-op final gate validation"
            )
            return []
        ...
        errors.append(
            f"migration_reports/migration_manifest.json could not be read: {exc}"
        )
        return []
    ...
    if not isinstance(raw_required_units, list) or not raw_required_units:
        errors.append(
            "migration_reports/migration_manifest.json must contain non-empty required_units"
        )
        return []
    ...
    if len(units) != len(required_units):
        errors.append(
            "migration_reports/migration_manifest.json required_units "
            "must contain only non-empty strings"
        )
        return []
    if len(set(units)) != len(units):
        errors.append(
            "migration_reports/migration_manifest.json required_units must not contain duplicates"
        )
        return []
    return units
...
def _extract_inventory_entries(value: object) -> dict[str, Mapping[object, object]]:
            ...
            for key, item in inventory.items():
                ...
                if isinstance(item, Mapping):
                    ...
                    if "name" not in entry:
                        entry["name"] = (
                            _first_string_field(entry, ("unit_identity", "name")) or key
                        )
    ...
    for item in entries:
        ...
        name = _first_string_field(
            entry, ("unit_identity", "name", "operator", "op_name", "row_id", "id")
        )
        if name:
...
def _validate_native_inventory_entry(
    entry: Mapping[object, object], label: str, errors: list[str]
) -> None:
    missing_fields = [
        field_name
        for field_name in (
    ...
    if missing_fields:
        errors.append(
            label + " missing native inventory fields: " + ", ".join(missing_fields)
        )
    _validate_fine_grained_inventory_entry(entry, label, errors)


def _validate_fine_grained_inventory_entry(
    entry: Mapping[object, object], label: str, errors: list[str]
) -> None:
    missing_fields = [
        field_name
    ...
    if missing_fields:
        errors.append(
            label + " missing fine-grained unit fields: " + ", ".join(missing_fields)
        )
    ...
    if granularity in COARSE_GRANULARITY_VALUES:
        errors.append(
            label + ".inventory_granularity must be fine_grained, not " + granularity
        )
    elif (
        entry.get("inventory_granularity") is not None
        and granularity not in FINE_GRAINED_GRANULARITY_VALUES
    ):
    ...
    for field_name in (
        ...
        if isinstance(value, Mapping):
            errors.append(
                f"{label}.{field_name} must describe one fine-grained unit, "
                f"not nested family/group mappings"
            )

    unit_identity = _first_string_field(entry, ("unit_identity",))
    row_name = _first_string_field(
        entry, ("name", "operator", "op_name", "row_id", "id")
    )
    if unit_identity and row_name and unit_identity != row_name:
        errors.append(
            label
            + ".unit_identity must match the source_inventory/manifest row identity"
        )
...
def _has_non_empty_inventory_value(value: object) -> bool:
    ...
    if isinstance(value, list):
        return any(
            _has_non_empty_inventory_value(item) for item in cast(list[object], value)
        )
    if isinstance(value, tuple):
        return any(
            _has_non_empty_inventory_value(item)
            for item in cast(tuple[object, ...], value)
        )
    if isinstance(value, set):
        return any(
            _has_non_empty_inventory_value(item) for item in cast(set[object], value)
        )
...
def _validate_source_inventory_metadata(value: object, errors: list[str]) -> None:
    if not isinstance(value, Mapping):
        errors.append(
            "source_inventory must include discovery_complete "
            "and discovery_sources_checked metadata"
        )
        return
    inventory = cast(Mapping[object, object], value)
    ...
    if "out_of_scope_source_groups" not in inventory:
        errors.append(
            "source_inventory.out_of_scope_source_groups must list excluded source groups, "
            "even when empty"
        )
    ...
    if not isinstance(sources, list):
        errors.append(
            "source_inventory.discovery_sources_checked must list source discovery categories"
        )
        return
    source_values = {
    ...
    if "requirements_doc" in source_values:
        errors.append(
            "source_inventory.discovery_sources_checked must be source-driven and "
    ...
    if missing_sources:
        errors.append(
            "source_inventory.discovery_sources_checked missing required sources: "
...
def _extract_row_name(row: Mapping[object, object]) -> str | None:
    return _first_string_field(
        row, ("unit_identity", "name", "operator", "op_name", "row_id", "id")
    )


def _first_string_field(
    row: Mapping[object, object], fields: tuple[str, ...]
) -> str | None:
...
def _validate_project_local_artifact(
    ...
    if not isinstance(value, Mapping):
        errors.append(
            f"rows[{index}].opp_custom_op_artifact_evidence must be an object "
            f"with project-local proof"
        )
        return
    evidence = cast(Mapping[object, object], value)
    if (
        evidence.get("project_local") is not True
        and evidence.get("in_project") is not True
    ):
        errors.append(
            f"rows[{index}].opp_custom_op_artifact_evidence must prove "
            f"project-local artifact creation"
        )
    if not _has_project_local_path_proof(evidence):
        errors.append(
            f"rows[{index}].opp_custom_op_artifact_evidence must include project-local path proof"
        )
    if not _has_positive_boolean(evidence, ("built", "present", "loaded", "verified")):
        errors.append(
            f"rows[{index}].opp_custom_op_artifact_evidence must prove artifact was built/loaded"
        )
    if _is_python_shim_artifact(evidence):
        errors.append(
            f"rows[{index}].opp_custom_op_artifact_evidence must not be a Python shim "
            f"or Python binding surface"
        )
    ...
    if not native_paths:
        if platform_policy is not None and platform_policy.id != "npu_ascend":
            ...
            errors.append(
                f"rows[{index}].opp_custom_op_artifact_evidence must prove a native compiled "
                f"{native_label} custom-op artifact"
            )
        else:
            errors.append(
                f"rows[{index}].opp_custom_op_artifact_evidence must prove a native "
                f"compiled Ascend custom-op artifact"
            )
    elif not _runtime_loaded_native_artifact_matches(
        evidence, native_paths, project_root
    ):
        errors.append(
            f"rows[{index}].opp_custom_op_artifact_evidence must prove the same-run "
            f"runtime loaded the native compiled artifact, not a Python shim"
        )
    ...
    if project_root is not None and native_paths:
        ...
        if not existing_paths:
            errors.append(
                f"rows[{index}].opp_custom_op_artifact_evidence native artifact path "
                f"must exist under the project root and be a non-empty compiled binary"
            )
...
def _validate_integration_route(value: object, index: int, errors: list[str]) -> None:
    if not isinstance(value, Mapping):
        errors.append(
            f"rows[{index}].integration_e2e_evidence must be an object with project API proof"
        )
        return
    evidence = cast(Mapping[object, object], value)
    if _mapping_is_disallowed_surrogate(evidence):
        errors.append(
            f"rows[{index}].integration_e2e_evidence must not be "
            f"synthetic/monkeypatch/report/benchmark-only"
        )
    if not _has_positive_boolean(
        ...
        errors.append(
            f"rows[{index}].integration_e2e_evidence must prove "
            f"public/project API custom-op execution"
        )
    if not _has_positive_boolean(
        evidence,
        (
        ...
        errors.append(
            f"rows[{index}].integration_e2e_evidence must prove "
            f"native compiled custom-op route execution"
        )
...
def _validate_runtime_coverage(value: object, index: int, errors: list[str]) -> None:
    if not isinstance(value, Mapping):
        errors.append(
            f"rows[{index}].same_run_runtime_coverage must be an object with runtime proof"
        )
        return
    evidence = cast(Mapping[object, object], value)
    if _mapping_is_disallowed_surrogate(evidence):
        errors.append(
            f"rows[{index}].same_run_runtime_coverage must not be "
            f"synthetic/monkeypatch/report/benchmark-only"
        )
    if evidence.get("same_run") is not True:
        errors.append(
            f"rows[{index}].same_run_runtime_coverage must prove same-run coverage"
        )
    if not _has_positive_boolean(
        evidence, ("project_api_route", "public_api_route", "custom_op_route_executed")
    ):
        errors.append(
            f"rows[{index}].same_run_runtime_coverage must prove runtime coverage "
            f"through the project API route"
        )
    if not _has_positive_boolean(
        evidence,
        (
        ...
        errors.append(
            f"rows[{index}].same_run_runtime_coverage must prove "
            f"native compiled custom-op runtime coverage"
        )
...
def _validate_performance(
    ...
    if not isinstance(value, Mapping):
        errors.append(
            f"rows[{index}].performance_evidence must be an object with numeric timings"
        )
        return
    evidence = cast(Mapping[object, object], value)
    if _mapping_is_disallowed_surrogate(evidence):
        errors.append(
            f"rows[{index}].performance_evidence must not be report-only or "
            f"benchmark-only without project API proof"
        )
    ...
    if perf_mode in ("full", "presence_only"):
        ...
        missing = [
            ...
            if not _positive_number(evidence.get(field_name))
        ]
        if missing:
            errors.append(
                f"rows[{index}].performance_evidence missing positive numeric fields: "
                + ", ".join(missing)
            )
    if perf_mode == "full":
        if not _positive_number(evidence.get("speedup_vs_baseline")):
            errors.append(
                f"rows[{index}].performance_evidence.speedup_vs_baseline must be a positive number"
            )
    ...
    if not _has_positive_boolean(
        ...
        errors.append(
            f"rows[{index}].performance_evidence must prove timing came from "
            f"public/project API custom-op route"
        )
    _validate_baseline_and_custom_device_proof(
        evidence, f"rows[{index}].performance_evidence", errors, platform_policy
    )
...
def _validate_performance_report_completeness(
    ...
    if not _performance_report_path_proves_required_file(report_map):
        errors.append(
            "performance_report must prove migration_reports/performance.json was written"
        )
    ...
    if not _has_positive_boolean(
        ...
        errors.append(
            "performance_report must prove speedup timings came from "
            "public/project API custom-op routes"
        )
    _validate_baseline_and_custom_device_proof(
        report_map, "performance_report", errors, platform_policy
    )
    ...
    if row_names:
        ...
        if not report_names:
            errors.append(
                "performance_report must contain per-unit speedup entries for every manifest row"
            )
            return
        if report_names != row_names:
            ...
            if extra:
                details.append(
                    "performance entries without manifest rows: " + ", ".join(extra)
                )
            errors.append(
                "performance_report must match manifest rows ("
                + "; ".join(details)
                + ")"
            )
        for unit_name, entry in report_entries.items():
            _validate_performance_report_entry(
                entry, unit_name, errors, platform_policy
            )
...
def _validate_overall_performance_report(
    ...
    if perf_mode == "full":
        ...
        missing = [
            field_name
            for field_name in required_positive
            if not _positive_number(report.get(field_name))
        ]
        if missing:
            errors.append(
                "performance_report missing positive overall speedup fields: "
                + ", ".join(missing)
            )
        ...
        if missing:
            errors.append(
                "performance_report missing positive overall timing fields: "
                + ", ".join(missing)
            )

    route_proven = _has_positive_boolean(
        report, ("overall_project_api_invoked", "overall_custom_op_route_executed")
    )
    all_units_replaced_proven = _has_positive_boolean(
        report,
        (
            "overall_all_units_replaced",
            "all_custom_op_units_replaced",
            "all_units_replaced",
        ),
    )
    ...
    if not route_proven:
        errors.append(
            "performance_report must prove overall timing ran through the project API "
            "after all custom-op units were replaced"
        )
    if not all_units_replaced_proven:
        errors.append(
            "performance_report must prove overall timing was measured "
            "after all source-discovered custom-op units "
            "were replaced"
        )


def _performance_report_path_proves_required_file(
    report: Mapping[object, object],
) -> bool:
    for field_name in ("path", "report_path", "project_relative_path"):
        value = report.get(field_name)
        if isinstance(value, str) and value.strip().replace("\\", "/").endswith(
...
def _extract_performance_report_entries(
    report: Mapping[object, object],
    ...
    for item in entries:
        ...
        entry = cast(Mapping[object, object], item)
        name = _first_string_field(
            entry, ("unit_identity", "row_id", "name", "operator", "op_name", "id")
        )
        if name:
            entries_by_name[name] = entry
    return entries_by_name
...
def _validate_performance_report_entry(
    ...
    if perf_mode in ("full", "presence_only"):
        ...
        missing = [
            field_name
            for field_name in required_positive
            if not _positive_number(entry.get(field_name))
        ]
        if missing:
            errors.append(
                f"performance_report.entries[{unit_name}] missing positive numeric fields: "
                + ", ".join(missing)
            )
    if perf_mode == "full":
        if not _positive_number(entry.get("speedup_vs_baseline")):
            errors.append(
                f"performance_report.entries[{unit_name}].speedup_vs_baseline "
                f"must be a positive number"
            )
    if not _has_positive_boolean(
        entry, ("project_api_invoked", "public_api_invoked", "custom_op_route_executed")
    ):
        errors.append(
            f"performance_report.entries[{unit_name}] must prove "
            f"public/project API custom-op timing route"
        )
    _validate_baseline_and_custom_device_proof(
        entry, f"performance_report.entries[{unit_name}]", errors, platform_policy
    )


def _validate_no_fallback_evidence(
    value: object, index: int, errors: list[str]
) -> None:
    if not isinstance(value, Mapping):
        errors.append(
            f"rows[{index}].no_fallback_no_zero_call_no_builtin_contamination "
            f"must be an object with explicit no-fallback proof"
        )
        return
    evidence = cast(Mapping[object, object], value)
    if _mapping_is_disallowed_surrogate(evidence):
        errors.append(
            f"rows[{index}].no_fallback_no_zero_call_no_builtin_contamination must not "
            f"be synthetic/monkeypatch/report/benchmark-only"
        )
    if any(evidence.get(field_name) is True for field_name in NEGATIVE_FALLBACK_FIELDS):
        ...
        return
    all_negative_flags_false = all(
        evidence.get(field_name) is False for field_name in NEGATIVE_FALLBACK_FIELDS
    )
    if not all_negative_flags_false:
        errors.append(
            f"rows[{index}].no_fallback_no_zero_call_no_builtin_contamination must "
            f"explicitly set all fallback/zero-call/builtin/baseline/stub flags to false"
        )


def _has_positive_boolean(
    evidence: Mapping[object, object], fields: tuple[str, ...]
) -> bool:
...
def _is_python_shim_artifact(evidence: Mapping[object, object]) -> bool:
    ...
    for field_name in (*_ARTIFACT_PATH_FIELDS, *_RUNTIME_LOADED_PATH_FIELDS):
        value = evidence.get(field_name)
        if isinstance(value, str) and value.strip().lower().replace("\\", "/").endswith(
            ".py"
        ):
            return True
...
def _native_artifact_path_candidates(evidence: Mapping[object, object]) -> list[str]:
    ...
    for field_name in _ARTIFACT_NESTED_FIELDS:
        ...
        if isinstance(value, Mapping):
            candidates.extend(
                _native_artifact_path_candidates(cast(Mapping[object, object], value))
            )
    return candidates
...
def _runtime_loaded_path_candidates(evidence: Mapping[object, object]) -> list[str]:
    ...
    for field_name in _ARTIFACT_NESTED_FIELDS:
        ...
        if isinstance(value, Mapping):
            candidates.extend(
                _runtime_loaded_path_candidates(cast(Mapping[object, object], value))
            )
    return candidates


def _paths_match(
    runtime_path: str, native_path: str, project_root: Path | None
) -> bool:
    if project_root is not None:
        ...
        if (
            native_resolved is not None
            and _container_runtime_path_matches_project_relative(
                runtime_path, native_path
            )
        ):
            return True
        return False
    return _normalize_reported_path(runtime_path) == _normalize_reported_path(
        native_path
    )


def _container_runtime_path_matches_project_relative(
    runtime_path: str, native_path: str
) -> bool:
    native_normalized = _normalize_reported_path(native_path)
    if (
        not native_normalized
        or native_normalized.startswith("/")
        or ".." in Path(native_normalized).parts
    ):
        return False
    runtime_normalized = runtime_path.strip().replace("\\", "/")
    return (
        runtime_normalized.endswith("/" + native_normalized)
        or runtime_normalized == native_normalized
    )
...
def _existing_native_artifacts(
    project_root: Path, native_paths: list[str]
) -> list[Path]:
    existing: list[Path] = []
    for native_path in native_paths:
...
def _resolve_existing_project_root(
    project_root: str | Path | None, errors: list[str]
) -> Path | None:
    if project_root is None:
        return None
    try:
        ...
        errors.append(
            f"project_root must exist for custom-op artifact validation: {exc}"
        )
        return None
    if not resolved.is_dir():
        errors.append(
            "project_root must be a directory for custom-op artifact validation"
        )
        return None
    return resolved
...
def _validate_build_provenance(
    ...
    if not isinstance(provenance, Mapping):
        errors.append(
            f"rows[{index}].opp_custom_op_artifact_evidence must include "
            f"build_provenance with command and project-local log_path"
        )
        return None
    ...
    if not isinstance(command, str) or not command.strip():
        errors.append(
            f"rows[{index}].opp_custom_op_artifact_evidence.build_provenance."
            f"command must be non-empty"
        )
    log_path = (
        provenance_map.get("log_path")
        or provenance_map.get("build_log_path")
        or provenance_map.get("cann_build_log_path")
    )
    if not isinstance(log_path, str) or not _is_safe_project_relative_path(log_path):
        errors.append(
            f"rows[{index}].opp_custom_op_artifact_evidence.build_provenance."
            f"log_path must be a safe project-relative path"
        )
        return None
    if project_root is not None:
        ...
        if resolved is None or not resolved.is_file() or resolved.stat().st_size <= 0:
            errors.append(
                f"rows[{index}].opp_custom_op_artifact_evidence.build_provenance."
                f"log_path must exist under the project root and be non-empty"
            )
            return None
        return resolved
    return None
...
def _validate_native_platform_evidence(
    ...
    build_log_text = (
        _read_text_limited(build_log_path) if build_log_path is not None else ""
    )
    build_log_has_native_evidence = _text_has_any_token(
        build_log_text, build_log_tokens
    )
    binary_has_native_evidence = any(
        _binary_has_platform_native_token(path, binary_tokens)
    ...
    if platform_policy is not None:
        ...
        binary_source_error = (
            platform_policy.custom_op_evidence.binary_source_error_message
        )
    else:
        build_log_error = (
            "must contain CANN/ACL/AscendC/OPP build or link evidence, "
            "not a torch-only extension build"
        )
        binary_source_error = (
    ...
    if not build_log_has_native_evidence:
        errors.append(
            f"rows[{index}].opp_custom_op_artifact_evidence."
            f"build_provenance.log_path {build_log_error}"
        )
    if evidence_only_paths:
        errors.append(
            f"rows[{index}].opp_custom_op_artifact_evidence must not use "
            f"evidence-only/stub/native-marker artifact or source paths: "
            f"{', '.join(sorted(evidence_only_paths))}"
        )
...
def _source_path_candidates(
    evidence: Mapping[object, object], row: Mapping[object, object]
) -> list[str]:
    candidates: list[str] = []
    for container in (evidence, row):
        for field_name in (
            ...
            if isinstance(value, str):
                ...
                for item in cast(
                    list[object] | tuple[object, ...] | set[object], value
                ):
                    if isinstance(item, str):
...
def _text_mentions_evidence_only_path(value: str) -> bool:
    ...
    for suffix in (
        ...
        for token in _EVIDENCE_ONLY_SOURCE_TOKENS:
            if (
                f"{token}{suffix}" in normalized
                or f"_{token}{suffix}" in normalized
                or f"-{token}{suffix}" in normalized
            ):
                return True
    return False
...
def _validate_baseline_and_custom_device_proof(
    ...
    if _has_diagnostic_baseline(evidence):
        errors.append(
            f"{label} must not use diagnostic-only or metadata-only baseline timings"
        )
    perf_mode = get_performance_validation_mode(platform_policy)
    if perf_mode == "disabled":
        return
    if not _has_baseline_proof(evidence, platform_policy):
        ...
        errors.append(
            f"{label} must prove timings include a baseline path "
            f"({', '.join(sorted(baseline_devices))})"
        )
    if not _has_target_device_custom_proof(evidence, platform_policy):
        errors.append(
            f"{label} must prove timings include a target-device custom-op path"
        )


def _has_diagnostic_baseline(evidence: Mapping[object, object]) -> bool:
    if any(
        evidence.get(flag_name) is True
        for flag_name in (
            "diagnostic_only",
            "baseline_diagnostic_only",
            "baseline_missing",
            "cuda_baseline_missing",
        )
    ):
        return True
...
def _has_baseline_proof(
    evidence: Mapping[object, object], platform_policy: PlatformPolicy | None = None
) -> bool:
...
def _has_device_value(
    evidence: Mapping[object, object], fields: tuple[str, ...], accepted: set[str]
) -> bool:
    for field_name in fields:
        value = evidence.get(field_name)
        if isinstance(value, str):
...
def _has_negative_contamination_signal(value: object) -> bool:
    ...
    return (
        isinstance(perf_value, (int, float))
        and not isinstance(perf_value, bool)
        and perf_value < 0
    )
