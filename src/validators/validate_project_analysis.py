...
def validate(data: dict[str, object]) -> ValidationDict:
    ...
    source_discovered_units = discover_required_cuda_native_units_from_project(project_dir)
...
def _requires_strict_custom_op_source_surface(
    surface: dict[str, object],
    source_discovered_units: list[NativeUnit],
    source_custom_op_indicators: list[CustomOpSourceIndicator],
) -> bool:
    if source_discovered_units or source_custom_op_indicators:
        return True
    if _has_active_expanded_variant_metadata(surface):
        return True
    for field_name in ("native_operator_symbols", "kernel_launch_sites"):
...
def _validate_fine_grained_unit_evidence(
    ...
    *,
    ...
    for index, item in enumerate(evidence_items):
        ...
        if require_route_evidence and not _has_non_empty_string_list(public_routes) and not _has_non_empty_string_list(framework_routes):
            errors.append(
