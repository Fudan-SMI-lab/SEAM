# src × 04_Deepwave 端到端迁移测试指南

## 概述

本文档说明如何使用 **src 管线**（而非手动操作）对 `04_Deepwave` 项目执行完整的自动迁移适配 + 端到端验证。
...
| `--output_dir` | `output_projects` | 指定迁移产物输出目录 |
...
[2026-04-21 10:XX:XX] [Iter 1/10] Running entry script...
[2026-04-21 10:XX:XX] [Iter 1/10] Validation FAILED (exit 1) - ImportError: ...
[2026-04-21 10:XX:XX] [Iter 1/10] Analyzer classified -> category=migration logic, role=code_adapter
...
...
python src/scripts/sm_adapt_cli.py \
  --project-dir original_projects/04_Deepwave \
...
- 确认 `--project-dir`、`--output_dir`、`--max-phase5-iter` 等参数
...
检查 `e2e-reports/src/YYYYMMDD_HHMMSS/phase_results.json`：

```bash
cat e2e-reports/src/YYYYMMDD_HHMMSS/phase_results.json \
  | python -m json.tool
```
