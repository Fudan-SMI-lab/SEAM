...
- For `vllm_serving` and `sglang_serving`, success is not valid until the route-specific serving final gate is current and `FULL_PASS`. The report must prove route/framework match, real project demo/test/API request validation, readiness probe success, Ascend NPU execution evidence, CANN env loaded, `torch_npu`/`tbe`/`te` import preflight, route framework import preflight, expected outputs, no CUDA/NVIDIA/NCCL runtime markers, no CUDA fallback, no CPU fallback, and fresh required report paths. Import-only and smoke-only serving checks are failures.
