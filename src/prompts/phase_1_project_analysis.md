...
- All five routes target Ascend. For `vllm_serving` and `sglang_serving`, the serving runtime surface must explicitly be Ascend-native: set `serving_backend="ascend"`, include `runtime_env_setup` for CANN `set_env.sh`/equivalent exports, CANN Python `PYTHONPATH` for `tbe` and `te`, CANN `LD_LIBRARY_PATH`, `torch_npu`, and route-specific Ascend serving packages. Include `required_import_probes` containing `torch`, `torch_npu`, `tbe`, `te`, and the selected framework (`vllm` or `sglang`). Include `ascend_runtime_checks` containing `cann_env_loaded`, `torch_npu_imported`, `tbe_imported`, `te_imported`, `<framework>_imported`, `cuda_nccl_markers_absent`, and `no_cpu_fallback`. Include `forbidden_runtime_markers` for CUDA/NVIDIA/NCCL paths such as `CUDA_VISIBLE_DEVICES`, `NVIDIA_VISIBLE_DEVICES`, `nvidia-smi`, `NCCL_`, `pynccl_allocator`, and `torch.cuda.memory`.
