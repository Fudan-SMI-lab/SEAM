...
## Context
This is a CUDA source-project migration workflow targeting the platform selected by platform policy. The selected command will become the target runtime validation surface after rule migration and repair. Original CUDA scripts may fail on the target accelerator at this stage; do not avoid CUDA-dependent paths because they fail before migration.
...
- Custom-op route without expanded variants: select or generate a fail-closed validation script contract. The script must fail at the target-runtime validation stage unless every fine-grained Phase 1 unit has corresponding target custom-op build/install provenance in the validation reports, strict `unit_identity` closure, runtime coverage, no-fallback evidence, and CPU-baseline vs target-custom performance evidence where platform policy requires it.
...
- SGLang serving route: return `entry_script_kind="sglang_serving_validation"`, `serving_framework="sglang"`, the same strict serving contract fields, and route/framework evidence tied to project-provided demo/test/API files. The framework will normalize this into a generated `validate_sglang_serving.py` wrapper.
...
```json
{
  ...
  "expanded_variant_inventory": {
    "unit_identities": ["source-derived-expanded-variant-identity"],
    "variant_axes_detected": true,
    "expanded_operator_instances_count": 1
  },
  "variant_axis_coverage": {
    "all_axes_covered": true,
    "covered_axes": ["source-discovered-axis-name"],
    "coverage_evidence": "source evidence proving every required variant axis is represented"
  },
  "per_variant_performance_report": {
    "one_entry_per_expanded_variant": true,
    "performance_entries_match_expanded_inventory": true
  },
...
- `expanded_variant_inventory`, `variant_axis_coverage`, `per_variant_performance_report`: required when Phase 1 reports `custom_op_with_variants` or expanded variants; these fields must be source-derived, cover every expanded variant identity and axis, and require one performance entry per expanded variant.
