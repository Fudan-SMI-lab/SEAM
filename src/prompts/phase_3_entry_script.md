...
- vLLM serving route: return `entry_script_kind="vllm_serving_validation"`, `serving_framework="vllm"`, `serving_backend="ascend"`, the real project launch command, readiness probe, request validation, project demo/test/API files, expected outputs, Ascend runtime env, serving reports dir, required report paths, required checks, and serving validation obligations. The framework will normalize this into a generated `validate_vllm_serving.py` wrapper; do not use inline shell env prefixes as `run_command`.
- SGLang serving route: return `entry_script_kind="sglang_serving_validation"`, `serving_framework="sglang"`, `serving_backend="ascend"`, the same strict serving contract fields, and route/framework evidence tied to project-provided demo/test/API files. The framework will normalize this into a generated `validate_sglang_serving.py` wrapper; do not use inline shell env prefixes as `run_command`.
- Serving contracts must include Ascend-specific fields: `runtime_env_setup`, `required_import_probes` (`torch`, `torch_npu`, `tbe`, `te`, plus `vllm`/`sglang`), `forbidden_runtime_markers` (CUDA/NVIDIA/NCCL markers including `pynccl_allocator` and `torch.cuda.memory`), and `ascend_runtime_checks` (`cann_env_loaded`, `torch_npu_imported`, `tbe_imported`, `te_imported`, framework imported, CUDA/NCCL markers absent, no CPU fallback).
