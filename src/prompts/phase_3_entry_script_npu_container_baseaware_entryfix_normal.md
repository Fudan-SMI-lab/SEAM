# Phase 3 - Entry Script Confirmation (Ascend NPU, Base-Env-Aware, Entry-Fix, Normal)
...
3. For serving-backed vLLM, SGLang, local OpenAI-compatible API, or local inference workflows, use or create a launcher that starts the server, waits for readiness, runs client validation, captures logs, and cleans up.
...
## Output Format
Return exactly one JSON object. Do NOT include any custom-op contract fields, these are omitted by route policy.
...
## Field Semantics
- `entry_script_path`: absolute host-visible path to the selected or created script. This path is readable by file tools, by Phase 3.5, and by the target execution backend after any path mapping.
...
- `phase5_entry_script_revision_allowed`: `true` means the target runtime phase may revise the entry script or command if validation finds the selected command or path is incorrect. Revision is bounded to finding a working entry that matches the project's actual migration target.
