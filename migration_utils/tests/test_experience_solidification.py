...
def test_classifier_routes_rule_prompt_document_and_skill():
    ...
    assert classifier.classify({"recommended_type": "rule", "title": "Replace cuda", "confidence": 0.8})["type"] == "rule"
    assert classifier.classify({"tags": ["prompt"], "problem_description": "Improve analyzer prompt"})["type"] == "prompt"
    assert classifier.classify({"category": "case_study", "title": "Migration case"})["type"] == "document"
...
def test_refiner_uses_solidifier_role_and_caches_per_role(tmp_path):
    ...
    for run_id in ["run-1", "run-2"]:
        refined = refiner.refine({
            ...
            "category": "cuda_api",
            "tags": ["cuda", "npu"],
        }, run_id, {}, classification=classification)
...
def test_refiner_uses_distinct_solidifier_sessions(tmp_path):
    store = ExperienceStore(str(tmp_path))
    session_mgr = FakeExperienceSessionManager()
    session_mgr.responses["experience_skill_solidifier"] = '{"type": "skill", "title": "Skill", "fix_steps": ["Do it"]}'
    session_mgr.responses["experience_document_solidifier"] = '{"type": "document", "title": "Doc", "body": "Knowledge"}'
    refiner = ExperienceRefiner(str(tmp_path / "artifacts"), store, session_mgr)

    refiner.refine({"candidate_id": "s", "title": "Skill"}, "run-s", {}, classification={
        "type": "skill",
    ...
    refiner.refine({"candidate_id": "d", "title": "Doc"}, "run-d", {}, classification={
        "type": "document",
...
def test_dispatcher_classifies_before_refining_without_llm(tmp_path):
    ...
    results = dispatcher.dispatch_and_refine("run-1", [{
        "candidate_id": "doc-1",
...
def test_nested_asset_writes_are_safe(tmp_path):
    ...
    store.write_refined_experience("run-1", {"title": "Nested"}, {
...
def test_non_skill_promotion_paths_and_catalog_assets(tmp_path):
    ...
    for exp_type, asset_name, expected_root in cases:
        path = store.promote_from_staging("run-1", exp_type, {
            "type": exp_type,
    ...
    index_types = {entry["type"] for entry in store.read_index() if entry.get("status") == "promoted"}
...
def test_legacy_skill_promotion_still_writes_expected_files(tmp_path):
    ...
    path = store.promote_from_staging("run-1", "skill", {
