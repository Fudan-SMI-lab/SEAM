...
migration_utils 提供了一套完整的 E2E 测试管线, 自动将 CUDA 项目适配到 Ascend NPU。本工具基于 Deepwave 成功运行 (`20260423_053647`) 的配置封装, 可直接用于后续的 Hallo、ChaiLab、Hallo3、InsectID、ChatPLUG、IndexTTS 等项目迁移。
...
> **注意**: 默认端口是 4098, 不是 4096。如果需要自定义, 使用 `--server-url`。
...
```bash
./scripts/run_e2e.sh 07_IndexTTS --server-url http://10.0.0.1:8080
```
...
```
migration_utils/e2e-reports/migration_utils/<YYYYMMDD_HHMMSS>/
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--server-url` | `http://127.0.0.1:4098` | OpenCode 服务器地址 |
...
python -c "
import json
r = json.load(open('migration_utils/e2e-reports/migration_utils/<latest>/summary.json'))
...
```bash
# 清理报告 (保留最近 5 个)
ls -td migration_utils/e2e-reports/migration_utils/*/ | tail -n +6 | xargs rm -rf
