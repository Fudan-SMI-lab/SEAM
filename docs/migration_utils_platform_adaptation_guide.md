...
| 入口 | 源文件 | 说明 |
|------|--------|------|
| `Orchestrator` | `migration_utils/core/orchestrator.py` | 经典编排器，依次执行 Phase 0→1、Phase 1.5、Phase 2→3、Phase 4、Phase 5、Phase 6 |
| `WorkflowExecutor` | `migration_utils/core/workflow_executor.py` | YAML 原生引擎，支持 `builtin`、`dispatch`、`loop`、`review`、`orchestration` 等 Phase 类型，含条件求值、钩子、遥测 |
...
Phase 5 是核心验证和修复循环，由 `migration_utils/core/repair_loop.py` 中的 `RepairLoopEngine` 实现：
...
工作流 YAML 文件位于 `migration_utils/workflows/`，由 `migration_utils/core/config.py` 中的 `load_workflow()` 加载。
...
Prompt 模板文件位于 `migration_utils/prompts/`，命名约定：
...
平台策略由 `migration_utils/core/platform_policy.py` 统一管理。这是一个 YAML 驱动的、纯数据类的策略系统，无需外部配置文件。
...
`custom_op_final_gate.json` 是入口脚在 Phase 5 中产出的关键报告。验证器 `validate_custom_op_final_gate`（位于 `migration_utils/validators/validate_validation_final.py`）对其进行结构化校验。
...
**修改文件**：`migration_utils/core/platform_policy.py`
...
**创建文件**：`migration_utils/workflows/{platform}_migration_v2_container.yaml`
...
**创建目录**：`migration_utils/prompts/`（所有模板在此目录下）
...
**修改文件**：`migration_utils/core/orchestrator.py`
...
**修改文件**：`migration_utils/tests/test_platform_policy.py`
...
**修改文件**：`migration_utils/tests/test_workflow_executor.py` 或新建
...
| 文件 | 说明 |
|------|------|
| `migration_utils/core/orchestrator.py` | 主编排器，串联 Phase 0→6 |
| `migration_utils/core/workflow_executor.py` | YAML 原生工作流执行引擎 |
| `migration_utils/core/phase_runner.py` | LLM Phase 执行器（PhaseRunner） |
| `migration_utils/core/repair_loop.py` | Phase 5 修复循环引擎（RepairLoopEngine） |
| `migration_utils/core/platform_policy.py` | 平台策略定义、预设、解析、token helper |
| `migration_utils/core/config.py` | 工作流 YAML 加载器 |
| `migration_utils/core/types.py` | 类型定义（PhaseDefinition、WorkflowDefinition 等） |
| `migration_utils/core/execution_backend.py` | 容器/local 执行后端 |

### 验证器

| 文件 | 说明 |
|------|------|
| `migration_utils/validators/validate_validation_final.py` | Phase 5 验证 + custom-op 最终证据门 |
| `migration_utils/validators/validate_entry_script.py` | Phase 3 入口脚本验证 |
| `migration_utils/validators/validate_entry_static.py` | Phase 3.5 静态验证 |
| `migration_utils/validators/validate_env_detect.py` | Phase 0 环境检测验证 |
| `migration_utils/validators/validate_project_analysis.py` | Phase 1 项目分析验证 |
| `migration_utils/validators/validate_venv.py` | Phase 2 虚拟环境验证 |
| `migration_utils/validators/validate_rule_migration.py` | Phase 4 规则迁移验证 |

### 工作流示例

| 文件 | 说明 |
|------|------|
| `migration_utils/workflows/ppu_migration_v2_auto_vllm018_smoke_baseaware_entryfix_keep.yaml` | PPU 生产工作流（custom-op 路线，entryfix prompt，含 CPU 基线） |
| `migration_utils/workflows/ppu_migration_normal_entry_057_experiment.yaml` | PPU 普通入口实验工作流（custom-op 禁用） |
| `migration_utils/workflows/npu_migration_v2_container.yaml` | NPU 容器工作流模板 |
| `migration_utils/workflows/npu_migration_v2.yaml` | NPU 本地工作流 |

### Prompt 模板

| 目录 | 说明 |
|------|------|
| `migration_utils/prompts/phase_0_env_detect*.md` | Phase 0 环境检测 prompt |
| `migration_utils/prompts/phase_1_project_analysis*.md` | Phase 1 项目分析 prompt |
| `migration_utils/prompts/phase_2_venv_create*.md` | Phase 2 虚拟环境创建 prompt |
| `migration_utils/prompts/phase_3_entry_script*.md` | Phase 3 入口脚本确定 prompt |
| `migration_utils/prompts/phase_35_static_validate*.md` | Phase 3.5 静态验证 prompt |
| `migration_utils/prompts/repair_*_fixer*.md` | Phase 5 修复 agent prompt |
| `migration_utils/prompts/phase_error_recovery*.md` | 错误分析 prompt |
| `migration_utils/prompts/phase_5_review*.md` | Review gate prompt |
| `migration_utils/prompts/phase_6_report*.md` | Phase 6 最终报告 prompt |

### 测试

| 文件 | 说明 |
|------|------|
| `migration_utils/tests/test_platform_policy.py` | 平台策略测试 |
| `migration_utils/tests/test_workflow_executor.py` | 工作流执行器测试 |
| `migration_utils/tests/test_phase_runner.py` | Phase 运行器测试 |
| `migration_utils/tests/test_repair_loop.py` | 修复循环测试 |
